					       ...
					       const char *got)
{
	...
	if (command)
		packet_client_error(writer, msg, command_name, command, got);
	else
		packet_client_error(writer, msg_nocmd, command_name, got);
}
					...
					const char *got)
{
	...
	static const char *msg = N_("%s: protocol error, "
				    ...
				    "got bad data '%s'");
	packet_client_error(writer, msg, command_name, command, function, got);
}
