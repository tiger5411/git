...
struct commit_list *get_shallow_commits(struct object_array_bare *heads, int depth,
		int shallow_flag, int not_shallow_flag)
{
	int cur_depth = 0;
	size_t i = 0;
	struct commit_list *result = NULL;
	struct object_array_bare stack = OBJECT_ARRAY_BARE_INIT;
	...
	while (commit || i < heads->nr || stack.nr) {
		...
		for (p = commit->parents, commit = NULL; p; p = p->next) {
			...
			if (p->next)
				object_array_bare_insert(&stack,
							 &p->item->object);
			else {
				commit = p->item;
				cur_depth = **commit_depth_at(&depths, commit);
