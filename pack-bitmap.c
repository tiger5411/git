...
static struct bitmap *find_objects(struct bitmap_index *bitmap_git,
				   ...
				   struct object_array_bare *roots,
				   struct bitmap *seen,
				   struct list_objects_filter_options *filter)
{
	...
	struct object_array_bare not_mapped = OBJECT_ARRAY_BARE_INIT;
	 ...
	 */
	for_each_object_array_bare_item(item, roots) {
		...
		object_array_bare_insert(&not_mapped, object);
	}
...
static struct bitmap *find_tip_objects(struct bitmap_index *bitmap_git,
				       struct object_array_bare *tip_objects,
				       enum object_type type)
{
	...
	for_each_object_array_bare_item(item, tip_objects) {
		struct object *object = item->item;
		int pos;
					 ...
					 int filter_provided_objects)
{
	...
	struct object_array_bare wants = OBJECT_ARRAY_BARE_INIT;
	struct object_array_bare haves = OBJECT_ARRAY_BARE_INIT;
	...
	for_each_object_array_entry(entry, &revs->pending) {
		...
		while (object->type == OBJ_TAG) {
			...
			if (object->flags & UNINTERESTING)
				object_array_bare_insert(&haves, object);
			else
				object_array_bare_insert(&wants, object);
	...
	object_array_bare_clear(&wants);
	object_array_bare_clear(&haves);
