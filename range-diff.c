...
static int read_patches(const char *range, struct string_list *list,
			...
			struct progress *progress, uint64_t *count)
{
...
static void get_correspondences(struct string_list *a, struct string_list *b,
				int creation_factor, struct progress *progress,
				uint64_t *count)
{
	ssize_t n = a->nr + b->nr;
		    ...
		    struct range_diff_options *range_diff_opts)
{
	...
	if (range_diff_opts->progress)
		progress = start_delayed_progress(_("Fetching data for range-diff"),
						  4);
	...
	if (progress) {
		...
		progress = start_delayed_progress(_("Computing range-diff"), 11);
	}

	if (!res) {
		find_exact_matches(&branch1, &branch2, progress, &count);
