...
test_expect_success 'checkout-index --temp correctly reports error on missing blobs' '
	...
	grep "unable to read sha1 file of file ($missing_blob)" stderr &&
	grep "unable to read sha1 file of symlink ($missing_blob)" stderr
'
...
test_expect_success 'checkout-index --temp correctly reports error for submodules' '
	...
	grep "cannot create temporary submodule sub" stderr
'
