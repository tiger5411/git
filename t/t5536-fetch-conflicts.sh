...
test_expect_success 'fetch conflict: config vs. config' '
	setup_repository ccc \
		...
		grep "fatal: Cannot fetch both refs/heads/branch1 and refs/heads/branch2 to refs/remotes/origin/branch1" error
	)
'
...
test_expect_success 'fetch conflict: criss-cross args' '
	setup_repository xaa \
		...
		git fetch origin \
			...
			refs/heads/branch2:refs/remotes/origin/branch1 2>error &&
		grep "warning: refs/remotes/origin/branch1 usually tracks refs/heads/branch1, not refs/heads/branch2" error &&
		grep "warning: refs/remotes/origin/branch2 usually tracks refs/heads/branch2, not refs/heads/branch1" error
	)
'
