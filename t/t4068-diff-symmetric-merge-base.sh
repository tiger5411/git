...
test_expect_success 'diff with no merge bases' '
	...
	grep "fatal: br2...br3: no merge base" err
'
...
do
	...
	test_expect_success "$cmd --merge-base with non-commit" '
		...
		grep "fatal: --merge-base only works with commits" err
	'

	test_expect_success "$cmd --merge-base with no merge bases and one commit" '
		git checkout main &&
		test_must_fail git $cmd --merge-base br3 2>err &&
		grep "fatal: no merge base found" err
	'

	test_expect_success "$cmd --merge-base with multiple merge bases and one commit" '
		git checkout main &&
		test_must_fail git $cmd --merge-base br1 2>err &&
		grep "fatal: multiple merge bases found" err
	'
done
...
test_expect_success 'diff-tree --merge-base with one commit' '
	...
	grep "fatal: --merge-base only works with two commits" err
'

test_expect_success 'diff --merge-base with range' '
	test_must_fail git diff --merge-base br2..br3 2>err &&
	grep "fatal: --merge-base does not work with ranges" err
'
