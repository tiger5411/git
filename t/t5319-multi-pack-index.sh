...
test_expect_success 'warn on improper hash version' '
	...
	(
		...
		git log -1 2>err &&
		grep "multi-pack-index hash version 2 does not match version 1" err
	) &&
	(
		cd sha256 &&
		mv ../mpi-sha1 .git/objects/pack/multi-pack-index &&
		git log -1 2>err &&
		grep "multi-pack-index hash version 1 does not match version 2" err
	)
'
