...
#
# It can also take a <common-fn> prefix along with
...
#
#
# Both of the conditions will be run to assert that the "want" variant
# doesn't pass yet, and that the <expect> variant describes the
# current behavio. Therefore neither of them can mutate the test
# state. I.e. they must be read-only commands like "test", not "rm".
...
# "test_expect_success". Use it like:
#
...
# the underlying "test_todo" function.
todo_test_cmp () {
	test "$#" -ne 3 && BUG_ARITY todo_test_cmp 3 $#
	local want=$1 &&
	local expect=$2 &&
	local actual=$3 &&
