...
static struct string_list *get_bundle_uri_using_list(struct transport *transport);

static struct string_list *get_bundle_uri(struct transport *transport)
{
	get_helper(transport);
	...
	return get_bundle_uri_using_list(transport);
}

static struct string_list *get_bundle_uri_using_list(struct transport *transport)
{
	struct helper_data *data = transport->data;
	struct string_list *bundle_uri = xcalloc(1, sizeof(struct string_list));
	fprintf(stderr, "hi %p\n", data);
	return bundle_uri;
}
