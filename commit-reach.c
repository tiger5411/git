...
int can_all_from_reach_with_flag(struct object_list *from,
				 ...
				 timestamp_t min_generation)
{
	...
	for_each_object_list_item(item, from) {
...
{
	struct object_list from_objs = OBJECT_ARRAY_INIT;
	...
	while (from_iter) {
		object_list_insert(&from_objs, &from_iter->item->object);
